...
% We can replace the missing values in your original data with the predicted values if needed.
% Initialize a complete_trait_matrix with NaN values
complete_trait_matrix = NaN(size(data1));
...
for i = 1:size(Y, 2)
    ...
    % Predict the current trait for missing data points
    predicted_values = predict(mdl, X(missing_indices, :));

    % Store predicted values in the complete_trait_matrix
...
% 'reduced_data' now contains the data reduced to the selected number of 
